...
describe('useEventFiltering', () => {
  it('filters events by rink, date, and category', () => {
    ...
    const { result } = renderHook(() => useEventFiltering(events, filterSettings, selectedRinkId), { wrapper });
    expect(result.current.length).toBe(1);
    expect(result.current[0].rinkId).toBe('ice-ranch');
  });
});
