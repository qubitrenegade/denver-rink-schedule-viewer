...
global.fetch = vi.fn((input: RequestInfo) => {
  // Return event data for facility .json files (but not -metadata.json)
  if (typeof input === 'string' && input.endsWith('.json') && !input.includes('-metadata')) {
  ...
  // Return metadata for facility -metadata.json files
  if (typeof input === 'string' && input.endsWith('-metadata.json')) {
    return Promise.resolve(new Response(
      JSON.stringify({
