...
export class BigBearScheduler {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === '/schedule' || request.method === 'GET') {
      ...
      return new Response(
        `Big Bear Worker - Scheduling alarm for ${nextAlarmTime.toISOString()}`,
    ...
    return new Response('Big Bear Scheduler - Use GET to schedule, POST to run manually, /status for info', {
      headers: ScraperHelpers.corsHeaders()
    });
  }

  async alarm(): Promise<void> {
    console.log(`⏰ Big Bear alarm triggered at ${new Date().toISOString()}`);
    ...
    try {
      ...
      const nextAlarmTime = ScraperHelpers.getNextScheduledTime(360);
      await this.state.storage.setAlarm(nextAlarmTime);
      console.log(`📅 Next Big Bear alarm scheduled for ${nextAlarmTime.toISOString()}`);
      
    } catch (error) {
      console.error('❌ Big Bear alarm failed:', error);
